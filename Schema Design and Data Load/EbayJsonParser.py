...
def handleUnterminated(s):
    ...
    if s[0] == '"' and s[-1] == '"':
        s = s[1:-1]
    #Insert a double quote at an unterminated quote
    ind = 0
    while ind < (len(s)):
        if s[ind] == '"':
            s= s[0:ind] + '"' + s[ind:]
...
def parseJson(json_file):
    ...
    with open(json_file, 'r') as f:
        ...
        for item in items:
            ...
            item_name = handleUnterminated(item_name)
            seller_id = item[u'Seller'][u'UserID']
            seller_id = handleUnterminated(seller_id)
		    ...
		    BidderID = handleUnterminated(BidderID)
    ...
    writeItemTable(ItemList, "Item.dat")
    writeBidTable(BidList, "Bid.dat")
    writeCategoryTable(CategoryList, "Category.dat")
    writeItemCatTable(ItemCatList, "ItemCategory.dat")
    writeUserTable(UserList, "User.dat")
